...
# # group by period types and quarters
 df <- df %>% 
   dplyr::group_by(fundingagency,indicator, period, period_type) %>% 
   dplyr::summarise(value = sum(value))
 
 df<-df%>%
   mutate(Period_Types = glue("{`period`} {`period_type`}"))
   paste0("period","period_type")
 
 df1 <- df %>% 
   dplyr::group_by(fundingagency,indicator, period, Period_Types) %>% 
   dplyr::summarise(value = sum(value))
...
df <- df_raw %>%
  ...
  tidyr::pivot_wider(names_from = period,
              values_from = value) %>% 
   mutate(`FY2`0` Achieved` =  fy2020cumulative/fy2020_targets) %>% 
...
So now that we have our table in the basic format we want, let's add the shading based on value which is analgous to conditional
formatting
